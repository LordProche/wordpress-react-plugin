...
class eight29_filters {
  ...
  public function get_filter_list($term, $post_type, $options) {
    $data = [];
    ...
    if ($term === 'date') {
      ...
      $data_array = [];
			$date_format = isset($options['date_format']) && $options['date_format'] === 'month' ? 'Y-m' : 'Y';
			$count = 1;

      foreach($dates as $date) {
        array_push($data_array, get_the_date($date_format, $date->ID));
      }

      $data_array = array_unique($data_array);
      
      foreach($data_array as $id => $date) {
				if(isset($options['date_format']) && $options['date_format'] === 'month') { //store as a month
				...
				else { //store as a year
					$year = $date;
					$start = date('Y-m-d\TH:i:s', strtotime(date($year.'-01-01')));
					$end = date('Y-m-t\TH:i:s', strtotime(date($year.'-12-31')));
        ...
        if ($post_type === $data['post_type']) {
          foreach ($post_type_value as $term => $term_value) { //taxonomy
            ...
            if ($meta_query) {
              ...
              if (count(eight29_filters::get_filter_list($term, $data['post_type'], $term_value)) > 0) { //remove taxonomies with 0 terms
                $terms[$term]['terms'] = eight29_filters::get_filter_list($term, $data['post_type'], $term_value);
              }
            }
          }
        }
